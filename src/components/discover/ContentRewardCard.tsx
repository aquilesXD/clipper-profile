interface ContentRewardCardProps {
  reward: {
    id: number
    title: string
    creator: string
...
export function ContentRewardCard({ reward }: ContentRewardCardProps) {
  return (
    <div className="bg-[#1a1a1a] rounded-lg border border-[#2a2a2a] overflow-hidden hover:border-[#3a3a3a] transition-colors">
      <div className="p-4">
        ...
        <p className="text-white font-medium mb-3 hover:underline cursor-pointer line-clamp-1">{reward.title}</p>

        <div className="flex justify-between mb-1">
          <p className="text-sm text-white">
            {reward.paidAmount} de {reward.totalAmount} pagado
          </p>
          <p className="text-sm text-white">{reward.percentage}%</p>
        </div>

        <div className="h-2 w-full bg-[#121212] border border-[#2a2a2a] rounded mb-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <div>
            ...
            <div className="flex space-x-1">
              {reward.platforms.includes("tiktok") && (
                <img
              ...
              {reward.platforms.includes("instagram") && <span className="text-pink-400 text-lg">📸</span>}
              {reward.platforms.includes("youtube") && <span className="text-red-500 text-lg">▶️</span>}
              {reward.platforms.includes("twitter") && <span className="text-blue-400 text-lg">𝕏</span>}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="bg-gradient-to-b from-[#1754D8] to-[#578CFF] text-white text-xs font-semibold py-1 px-2 rounded border border-[#2862e2]">
              {reward.rate.split(" / ")[0]} <span className="opacity-60">/ 1 mil</span>
            </div>
          </div>
        </div>
      </div>
